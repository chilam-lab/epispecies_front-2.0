        ...
        next: (response) => {
          ...
          // // Example Usage:
           for (const key of keys) {
            ...
            let climateNodesByCategory: { [key: string]: TreeNode[] } = {};

            // Build the object with separate arrays for each category
            keys.forEach(key => {
              climateNodesByCategory[key] = [{
                label: nameCategories[key],
                key: key,
                expanded: true,
                children: categoriesFilterList[key].map((item, index) => {
                  return {
                    label: item[0],
                    key: `${key}-${index + 1}`,
                    leaf: true,
